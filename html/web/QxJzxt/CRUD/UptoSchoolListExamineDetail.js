...
render(
    function () {
        ...
        groupContent3.push(hide("collegExamineStatus", model.data.statusid));
        ...
        if (model.data.statusid == 1 || model.data.statusid == 2 || model.data.statusid == 3) {//学院级操作
            groupContent3.push(select("常用语", "ComonLanguage1", "/QxJzxt/CRUD/GetComonLanguageSelect", "infodatatype", 4, "", false));
        }
        groupContent3.push(input('分数', 'mark', model.data.mark, '4'));
        groupContent3.push(area("学院意见", "college_opinion", model.data.college_opinion, 1));
        if (model.data.upto_status_id != 5) {
            //groupContent3.push(button("上报到学校", 6, Color.green, function () {
            //    submitForm("/QxJzxt/CRUD/UptoSchool");
            //}));
            groupContent3.push(button("撤销出上报名单", '1:5', Color.green, function () {
                submitForm("/QxJzxt/CRUD/CancelUptoList");
            }));
        }
        ...
        groupContent4.push(select("常用语", "ComonLanguage2", "/QxJzxt/CRUD/GetComonLanguageSelect", "infodatatype", 4, "", false));
        groupContent4.push(area("学校意见", "school_opinion", model.data.school_opinion, 1));
        if (model.data.upto_status_id == 5) {
            groupContent4.push(button("保存学校审核意见", '1:5', Color.green, function () {
                submitForm("/QxJzxt/CRUD/SaveSchoolOpinion");
            }));
        }
...
"*");
function formReady() {
    $("#ComonLanguage1").change(function () {
        var index = Number($("#ComonLanguage1").val());
        var option = $("#ComonLanguage1").text().split(',');
        $("#college_opinion").html(option[index]);
    });
    $("#ComonLanguage2").change(function () {
        var index = Number($("#ComonLanguage2").val());
        var option = $("#ComonLanguage2").text().split(',');
