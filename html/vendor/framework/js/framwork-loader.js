...
function InitMenuEvent(selector,type,  full)
{
   // debugger
    //设置默认选择器
    ...
    var active = {
        setTop: function () {
            ...
            //----计算弹窗数据
            var topHeight = $.hasValue(full) ? 0 : 50;
            var leftWidth = $.hasValue(full) ? 0 : 250;
            var formWidth = $.hasValue(full) ? "100%" : ($(window).width() - leftWidth) + "px";
            var formHeight = $.hasValue(full) ? "100%" : ($(window).height() - topHeight - 45) + "px";
