    ...
    //dom.attr("name", p);//设置name属性
        ...
        //用户标识
            ...
            cookie: function (name, value, time) {
                if (time == undefined) {
                    time = "d" + _c.client.expire;//默认*天过期
                }
                //设置过期时间
                var str = time;
            ...
            parseurl: function (url, type) {
               // //debugger
                ...
                if (url.indexOf("@") > -1) {
                    ...
                    //report
                    result.destUrl = _c.client.report + "?id=" + url.replace("*r", "");
                    result.destType = "r";
                }
                    ...
                    //form 
                    result.destUrl = (_c.client.form + "?id=" + url.replace('*f', ""));
                    result.destType = "f";
                } else {
                    //$.log(type)
                    if (type != undefined) {
                        ...
                        //report
                        result.destUrl = _c.client.report+"?id=" + url;
                        result.destType = "r";    
                    }                   
                }
        ...
        //刷新界面-如果想整页刷新需要将ajaxOnly设为false
            refresh: function (ajaxOnly) {
                ...
                if ($.geturl().indexOf(_c.client.report) > -1) {//是报表？
        ...
        //编辑器
            ...
            q: function (name) {
               // //debugger 
                ...
                //特殊处理
                if (currntUrl.indexOf(_c.client.form) > 0 || currntUrl.indexOf(_c.client.report) > 0) {
            ...
            //        InitWorkFlow();
            //        //报表初始化
            //        if ($.geturl() === _c.client.report) {
            //            //id为报表的数据api地址

            //            InitReport($.q("id"));
            //        }
            //            //表单初始化
            //        else if ($.geturl() === _c.client.form) {
            //            //sdebugger 
            //            //id为js路径或自定义页面路径
            //            InitForm($.q("id"));
            //        }
            ...
            //            //首页初始化
            //        else if ($.geturl() === _c.client.homepage) {
            //            //加载菜单
            //            if (g_leftmenu) {
            //                menuStageTwo(uid);
            //            }
...
function loginRedirect(units, index) {
   ...
   // $.msg('即将以' + units.name[index] + "的身份进入系统", 0);
    ...
    if ($.hasValue(_c.client.bind)) {//跳转到绑定地址
        $.go(g_bind);
        return;
    }

    setTimeout(function () {
        window.location.replace(_c.client.homepage);
    }, 10);
}
