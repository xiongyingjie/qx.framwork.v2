    ...
    $.ajax({
        ...
        success: function (data, status) {
            ...
            search.keyup(function () {
                var keyword = _c.trimString($(this).val());
                $("#side-menu li.dropdown,.qx-menu-search").remove();
                if (_c.hasValue(keyword)) {
                    for (var i = 0; i < g_menuData.length; i++) {
                        //  debugger
                        var item = g_menuData[i];
                        // _c.log(item.text() + "contain" + that.val() + "=" + (item.text().indexOf(that.val()) > -1))
...
function InitMenuEvent(selector, type, positonInfo) {
    ...
    if (_c.hasValue(positonInfo.center) && _c.hasValue(positonInfo.width)) {
    ...
    var active = {
        setTop: function () {
            var that = $(this);
            var url = _c.parseurl(that.attr('data-url')).destUrl;
            var id = _c.random();
            var title = that.data('title') ? that.data('title') : '加载中...';
            //----计算弹窗数据
            var topHeight = _c.hasValue(positonInfo.y) ? positonInfo.y : 0;
            var leftWidth = _c.hasValue(positonInfo.x) ? positonInfo.x : 0;
            var formWidth = _c.hasValue(positonInfo.width) ? ($.isString(positonInfo.width) ? positonInfo.width : positonInfo.width + "px") : "100%";
            var formHeight = _c.hasValue(positonInfo.height) ? ($.isString(positonInfo.height) ? positonInfo.height : positonInfo.height + "px") : "100%";
    ...
    $(selector).each(function (i, o) {
        ...
        var old = _c.hasValue(obj.attr('data-url')) ? obj.attr('data-url') : obj.attr("href");
        ...
        if (old.indexOf("*d") > -1 || old.indexOf("@") > -1) {
            ...
            obj.on("click", function () {
                // debugger
                var apiObj = $(this);
                var _destUrl = _c.hasValue(apiObj.attr('data-url')) ? apiObj.attr('data-url') : apiObj.attr("href");
                var dest = _c.parseurl(_destUrl);
                old = old.replace("*d", "");
                if (old.indexOf("(") > -1 && old.indexOf(")") > -1) { //函数
                    ...
                    if (dest.destUrl.indexOf("@download") > -1) {//下载文件
                        $.Ajax({
                            ...
                            error: function (data) {
                                if (g_debug) {
                                    $.alert("操作失败,请检查服务器是否提供了对应的api:<br/>" + _c.url(url), 2);
                                } else {
                                    $.alert("操作失败", 5);
                                }
                            }
...
function subSetTitle(title, index) {
    ...
    // _c.log(window.parent.document);
    _c.log(window.parent.setTitle(title, father.data("index")));
...
function InitWorkFlow() {
    ...
    $.Ajax({
        ...
        success: function (data, code, msg, url) {     //回调函数,result,返回值
            //_c.log(data);
            if (_c.hasValue(data.All) && (data.All.length > 0)) {
                _c.log("您有" + data.All.length + "条新任务,请您及时处理", 0);
...
function QxGrid() {
    ...
    this.InitReport = function (id) {
        ...
        $.ajax({
            ...
            error: function () {
                if (_c.isDebug) {
                    _c.warn("未能加载grid的附加js,请检查项目目录下是否存在" + jsurl);
                }
            }
        });
        //debugger
                ...
                if (typeof g_deleteLink === "undefined") {
                    ...
                    if (_c.isDebug) {
                        $.confirm("获取报表搜索条件过程中出现错误,查看服务器返回数据的详细数据？", function () {
                            $.window(_c.url(id), "服务器返回数据详情");
                            return true;
                        });
                    }
                    return;
                }
                if (!_c.hasValue(g_deleteLink)) {
                    $("#bt_delete").remove();
                }
                if (!_c.hasValue(g_addLink)) {
                    $("#bt_add").remove();
                } else {
                    //b绑定导入excel的事件
    ...
    var UpdateQueryParam = function () {
            ...
            case 204:
                {
                    ...
                    value = _c.hasValue(selecteditem) ? selecteditem.value : ";";
                };
                break;
            default:
                {
        ...
        //前方插入固定参数
        g_params = (_c.hasValue(g_fixedParams) ? g_fixedParams + ";" : "") + queryParam;
                ...
                $.Ajax({
                    ...
                    success: function (data, code, msg, url) {
                        ...
                        if (chkValues.length === trueCount + falseCount) {
                            $.msg(_c.hasValue(msg) ? msg : '已成功将' + trueCount + '条记录' + tip, code);
                ...
                for (var j = 1; j < tableBody.length; j++) {
                    ...
                    for (var k = 0; k < tableBody[j].length; k++) {
                        if (k === tableBody[j].length - 1) {
                            ...
                            //debugger 
                            html.push('<td  style="text-align:center; class="center overOut">' + _c.replace(tableBody[j][k], ["[]", g_extraParam]) + '</td>');
                        } else {
 ...
 */});
    ...
    var InputTpl = function (label, name, value, num, reg) {
        ...
        //debugger 
        return _c.qx_tpl({ reg: reg, label: (_c.hasValue(label) ? " <label>" + label + "</label>" : ""), name: name, value: value, number: number, tip: (_c.hasValue(tip) ? tip : "id") }, function () {/*
 ...
 */});
    ...
    var ShowInputTpl = function (label, name, value, num) {
        ...
        value = check(value, "");
        return _c.qx_tpl({ name: name, label: label, value: value, number: number }, function () {/*
 ...
 */});
    ...
    var TimeTpl = function (label, name, value, num, tip) {
        ...
        return _c.qx_tpl({ label: label, name: name, value: value, number: number, tip: tip }, function () {/*
 ...
 */});
    ...
    var ShowTimeTpl = function (label, name, value, num) {
        ...
        return _c.qx_tpl({ label: label, name: name, value: value, number: number }, function () {/*
 ...
 */});
    ...
    var SelectTpl = function (label, name, option, value, num, readonly) {
        ...
        return _c.qx_tpl({ label: label, name: name, value: value, number: number, readonly: readonly ? "disabled" : "", option: optionHtml, optionScript: optionScript }, function () {/*
 ...
 */});
    ...
    var RadioTpl = function (label, name, items, num, vertical, value) {
        ...
        return _c.qx_tpl({ label: label, number: number, itemHtml: itemHtml }, function () {/*
 ...
 */});
    ...
    var SwitchTpl = function (label, name, num, value, onText, offText) {
        ...
        if (!_c.hasValue(onText)) {
            onText = '开';
        }
        if (!_c.hasValue(offText)) {
            offText = '关';
        }

        return _c.qx_tpl({ name: name, label: label, number: number, value: value, onText: onText, offText: offText }, function () {/*
 ...
 */});
    ...
    var AreaTpl = function (label, name, value, num, tip, height) {
        ...
        return _c.qx_tpl({ height: height, label: label, name: name, value: value, number: number, tip: tip }, function () {/*
 ...
 */});
    ...
    var ShowAreaTpl = function (label, name, value, num, height) {
        ...
        height = check(height, 200);
        var number = 12 / check(num, 1);
        return _c.qx_tpl({ height: height, label: label, name: name, value: value, number: number }, function () {/*
 ...
 */});
    ...
    var RichBoxTpl = function (label, name, value, num, height) {
        ...
        var number = 12 / check(num, 1);
        return _c.qx_tpl({ label: label, name: name, value: value, number: number, height: height }, function () {/*
 ...
 */});
    ...
    var ImageTpl = function (type, value, num, tip) {
        ...
        return _c.qx_tpl({ type: type, value: value, number: number, tip: tip }, function () {/*
 ...
 */});
    ...
    var ButtonTpl = function (id, label, num, color, onclick) {
        ...
        return _c.qx_tpl({ id: id, label: label, number: number, offset: offset, color: color, onclick: (onclick !== undefined) ? onclick.toString() : "" }, function () {/*
 ...
 */});
    ...
    var TabTpl = function (tabId, cfg, num) {
        ...
        var number = 12 / check(num, 1);
        return _c.qx_tpl({ id: tabId, subTitle: subTitle, subContent: subContent, number: number }, function () {/*
 ...
 */});
    ...
    var GroupTpl = function (id, html, title, num) {
        ...
        return _c.qx_tpl({ id: id, title: title, formContent: formContent, number: number }, function () {/*
 ...
 */});
    ...
    var PreTpl = function PreTpl(id, code, num) {
        var number = 12 / check(num, 1);
        return _c.qx_tpl({ id: id, code: code, number: number }, function () {/*
 ...
 */});
    ...
    var DropDownTpl = function (id, label, li, num, color) {
        ...
        //_c.log(liHtml)
        return _c.qx_tpl({ id: id, label: label, liHtml: liHtml.join(''), color: color, number: number }, function () {/*
 ...
 */});
    ...
    var PanelTpl = function (id, title, body, footer, color, num) {
        ...
        var number = 12 / check(num, 1);
        //debugger 
        return _c.qx_tpl({ id: id, title: title, body: _render(body), footer: footer, color: color, number: number }, function () {/*
 ...
 */});
    ...
    var ListGroupTpl = function (id, title, libody, num) {
        ...
        return _c.qx_tpl({ id: id, title: title, liHtml: liHtml.join(""), number: number }, function () {/*
 ...
 */});
    ...
    var MediaTpl = function (id, imgurl, header, text, num) {
        var number = 12 / check(num);
        return _c.qx_tpl({ id: id, imgurl: imgurl, header: header, text: text, number: number }, function () {/*
 ...
 */});
    ...
    var AlertTpl = function (id, text, num, color) {
        var number = 12 / check(num, 1);
        return _c.qx_tpl({ id: id, text: text, number: number, color: color }, function () {/*
 ...
 */});
    ...
    var ThumbNailTpl = function (id, imgurl, body, num) {
        var number = 12 / check(num);
        return _c.qx_tpl({ id: id, imgurl: imgurl, body: body, number: number }, function () {/*
 ...
 */});
    ...
    var PageHeaderTpl = function (id, title, subtitle, num) {
        var number = 12 / check(num);
        return _c.qx_tpl({ id: id, title: title, subtitle: subtitle, number: number }, function () {/*
 ...
 */});
    ...
    var CarouselTpl = function (id, imageUrl, num) {
        ...
        return _c.qx_tpl({ id: id, liHtml: liHtml.join(""), imgHtml: imgHtml.join(""), number: number }, function () {/*
 ...
 */});
    ...
    var FileUploadTpl3rd = function (label, name, num, folder, url) {
        ...
        return _c.qx_tpl({ label: label, name: name, url: url, folder: folder, number: number }, function () {/*
 ...
 */});
    ...
    var FileItemTpl = function (item, name, canDelete) {
        item.remove = true;
        //_c.warn(item);
        var savedUrl = item.url; var id = _c.random();
        item.url = _c.url((item.url.indexOf("-split-") > -1) ? encodeURI(item.url) : "");
        item.name = _c.hasValue(item.name) ? item.name : $.getFileName(item.url);
        return ' <tr id="' + id + '" class="template-download fade in">' +
            ...
            '<td><span class="size">' + (_c.hasValue(item.size) ? item.size : "") + '</span></td>' +//文件大小
    ...
    var ShowFileTpl = function (id, label, files, num) {
        var oldValue = files;
        //没有文件直接终止
        //if (($.isArray(files)&&files.length == 0)||!_c.hasValue(files)) {
        //	return "";
        //}
        //读取旧值
        if ($.isArray(files)) {
        ...
        //debugger
        var number = 12 / check(num, 2); _c.log(files);
        return _c.qx_tpl({ id: id, label: label, bodyHtml: FileItemsTpl(files, id), number: number, oldValue: oldValue }, function () {/*
 ...
 */});
    ...
    var HideTpl = function (name, value) {
        ...
        return _c.qx_tpl({ name: name, value: value }, function () {/*
 ...
 */});
    ...
    var TableTpl = function (header, body, num, id) {
        //debugger 
        id = check(id, _c.random());
        var number = 12 / check(num, 1);
        var tableScript = "";

        var bodyHtml = "";
        var headerHtml = "";
        if (_c.hasValue(body) && $.isString(body) && !_c.hasValue(header)) {
            //传入的是url
            ...
            if (_c.hasValue(body) && !$.isArray(body)) {
                body = [body];
            }
            bodyHtml = "<tbody>";
        ...
        return _c.qx_tpl({ id: id, bodyHtml: bodyHtml, headerHtml: headerHtml, number: number, tableScript: tableScript }, function () { /*
 ...
 */});
    ...
    this.hides = function (nameArray, valueArray) {
        ...
        return push({ id: _c.random(), type: 115, html: _render(hideArray) });
    ...
    this.hideTimes = function (nameArray, valueArray) {
        ...
        return push({ id: _c.random(), type: 116, html: _render(hideArray) });
    ...
    this.showfile = function (label, name, files, num, folder, url) {
        ...
        name = _c.hasValue(name) ? name : _c.random();
            ...
            store.query(function (data) {//请求数据
                ...
                $.each(data, function (i, o) {
                    ...
                    if (_c.hasValue(row)) {
                        ...
                        //
                        /*if(!_c.hasValue(processedRow)||!$.isArray(processedRow)){
                            _c.warn("table的body函数没有返回正确的数组,已跳过索引为"+i+"的数据");
                            continue;
                        }else{
                            o=processedRow;
                        }*/
    ...
    */
    ...
    this.submitForm = function (submiturl, doAfterSuccess) {
        ...
        if (submiturl === "*") {
            ...
            url += _c.hasValue(submiturl) ? submiturl : $.getsubmiturl();
        }
        $.Ajax({
            url: url, //调用WebService的地址和方法名称组合 ---- WsURL/方法名
            ...
            if (obj.type == 201 || obj.type == 202 || obj.type == 203 || obj.type == 209) {
                ...
                if (typeof obj.validators != "undefined") {
                    ...
                    obj.validators.notEmpty = {
                        message: _c.hasValue(obj.tip) ? obj.tip : obj.label + ' 是必填项'
                    }

                    if (obj.validators.max != undefined) {
                        obj.validators.stringLength = {
                            max: obj.validators.max,
                            message: _c.hasValue(obj.tip) ? obj.tip : obj.label + '输入的最大长度为' + obj.validators.max + '个字符'
                        }
                    }
                    if (obj.validators.min != undefined) {
                        obj.validators.stringLength = {
                            min: obj.validators.min,
                            message: _c.hasValue(obj.tip) ? obj.tip : obj.label + '要求至少输入' + obj.validators.max + '个字符'
                        }
                    }
                    if (obj.validators.max != undefined && obj.validators.min != undefined) {
                        obj.validators.stringLength = {
                            min: obj.validators.min,
                            max: obj.validators.max,
                            message: _c.hasValue(obj.tip) ? obj.tip : obj.label + '要求输入' + obj.validators.min + '-' + obj.validators.max + '个字符'
                        }
                    }
                    if (obj.validators.email != undefined) {
                        obj.validators.emailAddress = {
                            message: _c.hasValue(obj.tip) ? obj.tip : obj.label + '要求输入正确邮箱地址,如：123@sever.com'
                        }
                    }
    ...
    this.render = function (htmlArray, submiturl, dataUrl, title, overWrite) {
        ...
        if (!$.isFunction(htmlArray)) {
            //新版1：传入的是json格式的ui配置
            if ($.isObject(htmlArray) && !_c.hasValue(submiturl)) {
                //debugger
                render2(htmlArray);
                return;
            }
            //新版2：传入的是内容系统tableId
            if ($.isString(htmlArray) && !_c.hasValue(submiturl)) {

                $.ajax({
                    url: _c.url("/open/gettable?id=" + htmlArray, true),
        ...
        //是否有绑定值
        if (_c.hasValue(dataUrl)) {
            // var url = _c.url($.getsubmiturl());
            $.Ajax({
                url: dataUrl, //参数1
                // data: q,
                success: function (data, code, msg, url) {
                    ...
                    } else if (!_c.hasValue(data)) {
                        _c.warn("从" + url + "获取的数据为空");
                    }
                    _c.log(data);
    ...
    this.InitForm = function (id) {
        ...
        if (!_c.hasValue(id)) {
            _c.warn("参数错误:没有指定目标form！"); return;
        }
        id = _c.route(id);
        var jsurl = id;//页面
        ...
        $.ajax({
            ...
            error: function () {
                _c.log("未能正确加载界面css,请检查项目的web目录下是否存在" + (cssurl));
            }
        });
...
function TaskTpl(url, title, time, note, progress) {
    ...
    return _c.qx_tpl({ url: url, title: title, time: time, note: note, progress: progress == 0 ? progress + 1 : progress }, function () { /*
...
function MsgTpl(url, title, time, note) {
    return _c.qx_tpl({ url: url, title: title, time: time, note: note }, function () {/*
