...
//#框架加载器
var g_moudel = "panda_site_clothes";
...
head.ready(["jquery", "layui"], function () {
    $.extend({
        ...
        //用户标识sub_system_reg_id
        appid: function () {
            return $.cookie("sub_system_reg_id");
        },
        //用户标识
        user_id: function () {
    ...
    $.Ajax({
        ...
        success: function (data, code) {
            if (code === 1) {
                ...
                $.cookie('sub_system_reg_id', data.sub_system_reg_id); 
                var units = $.toObject(data.units);
                chooseOrgToLogin(units);//选择组织机构
            } else {
                $.warn(data);
                $.alert("页面初始化失败,请关闭后重新打开！", 5);// $.alert("获取用户信息失败！", 5);
            }
        }
    });
}
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (_c.isDebug) {
                ...
                $.alert("页面初始化失败(code=4001),请关闭后重新打开！", 5);//网络连接失败,请检查您的网络连接状态code=4001
            }
