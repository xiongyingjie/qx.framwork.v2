var repair_man = "";
var charge_man = "";
$.bindPage("wx.bx.device_record@list".eq("state", 2).
...
<div class="card facebook-card">
                    ...
                    <div class="card-content">
                        <table>
                            ...
                            <tr>
                                <th>维修人</th>
                                <td id="repair_man_{index}"></td>
                            </tr>
                            <tr>
                                <th>巡检员</th>
                                <td id="charge_man1_{index}">{device_record-charge_man}</td>
                            </tr>
                        </table>
                      
                    </div>
                    <div class="card-footer">未处理<span style="color:rgb(68,146,235);"><a href="reexaminedetail.html?id={device_record_id}">安排回访人</a></span></div>
                </div>
              */
    }, function (data) {
        $.each(data, function (i, o) {
            ...
            repair_man = o.repair_man;
            charge_man = o.charge_man;
            ("/Json/Bx/GetUserInfo?userid=" + repair_man).query(function (data) {
                $("#repair_man_"+o.index).text(data);
            });
...
$.bindPage("wx.bx.device_record@list".in("state", [3,4,5]).
    jn("employ.user_id", "device_record.request_man").
    jn("device.device_id", "device_record.device_id").
    jn("address.address_id", "device.address_id").
    jn("hitch_type.hitch_type_id", "device_record.hitch_type_id"), [
...
<div class="card facebook-card">
                    ...
                    <div class="card-footer"> <span style="color:rgb(68,146,235);"></span><span>已回访</span></div>
                </div>
              */
    },function (data) {
        $.each(data, function (i, o) {
