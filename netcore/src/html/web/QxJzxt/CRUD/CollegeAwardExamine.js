...
render(
    function () {
        ...
        if (model.data.statusid == 1 || model.data.statusid == 0 ) {//学院级操作
            groupContent3.push(showinput("审核状态", "examinestatus" , "待审核", 4));
        }
        if (model.data.statusid == 2 || model.data.statusid == 4 || model.data.statusid == 5 || model.data.statusid == 6) {//学院级操作
            groupContent3.push(showinput("审核状态", "examinestatus", "审核通过", 4));
        }
        if (model.data.statusid == 3) {//学院级操作
            groupContent3.push(showinput("审核状态", "examinestatus", "审核不通过", 4));
        }
        ...
        if (model.data.statusid == 1 ) {//学院级操作
            groupContent3.push(button("通过", '1:5', Color.green, function () {
                submitForm("/QxJzxt/CRUD/CollegeAwardExamine?flag=2");
            }));
            groupContent3.push(button("驳回", '6:0', Color.orange, function () {
                submitForm("/QxJzxt/CRUD/CollegeAwardExamine?flag=3");
            }));
        }
