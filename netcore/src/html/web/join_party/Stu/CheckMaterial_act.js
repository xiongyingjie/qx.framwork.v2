...
hide('cultivation_object_info-unit_id'),
       ...
       panel([
		...
		panel([
			...
			],"志愿服务&nbsp;&nbsp;(<a class='qx-operate' data-url='*fjoin_party/Stu/serviceAdd'>添加</a>)",1,Color.red,"",1),
...
function refresh() {
    renderTable([
...

	    {
			...
			store:"ecampus.join_party.cultivation_object_info@list".
			      jn("volunteer_service.UserId").
			      eq('volunteer_service.UserId', '_uid'),
		    row :function(o, i, data) {
			    return [o.describe, o.upload_time,
				 [{ text: "下载证明材料", url:  "ecampus.join_party.volunteer_service@download&id=" + o.volunteer_service_Id + "&file=attachment"},
                  { text: "删除", url: "ecampus.join_party.volunteer_service@delete&id=" + o.volunteer_service_Id }
              ]] }
	    },//志愿活动
