...
render([
    group([
        ...
        select("转入状态", "Requst_table-cultivation_object_stateId",
			       [{ text: '积极分子', value: 2 },
                   { text: '发展对象', value: 3 },
                   { text: '预备党员', value: 4 },
                   { text: '正式党员', value: 5 }], "", 3),
        file("上传证明材料（主要为介绍信）", "Requst_table-intro_file", 1, "/UserFile/join_party/intro_file/"),
//---------------------------查出stu_info表中的列----------------
...
//---------------------------查出Requst_table表中的列-----------------------------
         ...
         hide('Requst_table-UserId'),
         hide('Requst_table-sub_time'),
         hide('Requst_table-state'),

         hide('Requst_table-join_party_Id'),//查出党支部id
         hide('Requst_table-attachment'),//查出党支部id
//---------------------------提交数据-----------------------------
         ...
         button("提交材料", '1:5', Color.green, function () {
             ...
             //$("#cultivation_object_info-cultivation_object_stateId").val($("#Requst_table-cultivation_object_stateId").val());
             //$("#cultivation_object_info-move_in_state").val("0");
             //$("#cultivation_object_info-birthday").val($("#stu_info-birthday").val());


             $("#Requst_table-sub_time").val("#now");
             $("#Requst_table-state").val("0");
             $("#Requst_table-Requst_id").val("#id");
             $("#Requst_table-UserId").val($("#stu_info-uid").val());
             $("#Requst_table-join_party_Id").val($("#league_branch-Party_branch_id").val());

             submitForm("ecampus.join_party.Requst_table@add");
         })
