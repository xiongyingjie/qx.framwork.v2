  ...
  public static  class Extensions
    {
      ...
      {
            db.account.Update(accountBag.Account);
        ...
        public static bool SyncPayOrder(this MyDbContext db, PayOrderBag payOrderBag)
        {
            ...
            {   // 同步代码块
                ...
                if (payOrderBag.IsSuccessful)
                {
                    ...
                    db.account.Update(payOrderBag.Payer.Account);
                    db.account.Update(payOrderBag.Receiver.Account);
                    db.pay_order.Update(payOrderBag.PayOrder);
                    return db.SaveChanges() > 0;
                }
                db.account.Update(payOrderBag.Payer.Account);
                db.account.Update(payOrderBag.Receiver.Account);
