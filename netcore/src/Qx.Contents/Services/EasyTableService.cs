...
namespace Qx.Contents.Services
{
    ...
    public class EasyTableService : IEasyTableService
    {
        ...
        public bool EidtTableRow_After(string jsonstring, string tableid, string relationkeyID)
        {
            ...
            for (int i = 0; i < oList.Count; i++)
            {
                ...
                content_column_value tempValue = columns[i].content_column_value.FirstOrDefault(b => b.relation_key_value == relationkeyID);
    ...
    {
        public UpdateColumnValueFailedException(string 更新列值失败)
        {
    ...
    {
        public GetTableValueFailedException(string 获取表格列值失败)
        {
            throw new NotImplementedException();
        }
    }
    ...
    {
        public TableNotFindException(string 未查找到该表格)
        {
            throw new NotImplementedException();
        }
    }
