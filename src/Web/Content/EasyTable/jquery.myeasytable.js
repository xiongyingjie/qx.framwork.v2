...
(function ($, window, i) {
    ...
    $.fn.editTable = function (options) {
        ...
        // Build row
        function buildRow(data, len, thtitle, b) {
            ...
            if (!s.row_template) {
                ...
                for (b = 0; b < (len || data.length) ; b += 1) {
                    rowcontent += '<td class="myselftd">' + thtitle + '</td>' + buildCell(data[b]);
                }
            } else {
                // With row template
                //for (b = 0; b < s.row_template.length; b += 1) {
                //    // For each field in the row
                //    rowcontent += '<th>' + thtitle + '</th>'+ buildCell(data[b], s.row_template[b]);
                //}

                rowcontent += '<td class="myselftd">' + thtitle + '</td>' + buildCell(data, s.row_template[b]);
            }
        ...
        function fillTableData(data) {
            ...
            if (s.headerCols || s.row_template) {
                ...
                //}
                $table.find('thead tr').append('<th>列名</th>');
                $table.find('thead tr').append('<th>数据</th>');

                crow = Math.min(s.maxRows, col.length);
                ...
                for (a = 0; a < crow; a += 1) {
                    ...
                    buildRow(data[a], col.length, col_title, a).appendTo($table.find('tbody'));
                }
        ...
        function exportData() {
            ...
            $table.find('tbody tr').each(function (tri, trv) {
                ...
                $(this).find('td').each(function (i, v) {
                    ...
                    if (i == 0)
                    {
                        value = $(this).text();
                            ...
                            if (s.row_template[tri] == "text") {
                                value = $(this).find('input[type="text"]').val()
                            }
                            else if (s.row_template[tri] == "textarea")
                            {
                                value = $(this).find("iframe").contents().find("body").html()
                            }
                            else {
                                var field = s.field_templates[s.row_template[tri]],
