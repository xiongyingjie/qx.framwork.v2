...
{
    public class PayOrderController : BaseOrderController,ICrud<PayOrder_M>
    {

        private IRepository<r_pay_order> _repository;
        public PayOrderController( IRepository<r_pay_order> repository)
        {
        ...
        public ActionResult Index(string ReportID, string Params, int pageIndex = 1, int perCount = 10)
        {
            ...
            {
                return RedirectToAction("Index", new { ReportID = "Djk.Order.支付订单", Params = Params });
            }

            //初始化报表
            InitReport("支付订单", "#","",true,"");
            return View();
        }
        ...
        [HttpPost]
        public ActionResult Create(PayOrder_M viewModel)
        {
            try
            {
        ...
        [HttpPost]
        public ActionResult Edit(PayOrder_M viewModel)
        {
            try
            {
                if (ModelState.IsValid)
