...
{
    public class OrderTypeController : BaseOrderController,ICrud<OrderType_M>
    {

        private IRepository<order_type> _repository;
        public OrderTypeController( IRepository<order_type> repository)
        {
        ...
        public ActionResult Index(string ReportID, string Params, int pageIndex = 1, int perCount = 10)
        {
            ...
            {
                return RedirectToAction("Index", new { ReportID = "Djk.Order.订单类型", Params = Params });
            }

            //初始化报表
            InitReport("订单类型", "#","",true,"");
            return View();
        }
        ...
        [HttpPost]
        public ActionResult Create(OrderType_M viewModel)
        {
            try
            {
        ...
        [HttpPost]
        public ActionResult Edit(OrderType_M viewModel)
        {
            try
            {
                if (ModelState.IsValid)
