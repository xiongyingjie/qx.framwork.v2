...
namespace Qx.Tools
{
    ...
    {
        public static void UploadBigFile(HttpPostedFile fileBase, string SavePath, string ContentRange)
        {
            //disk存储过程
        ...
        {
            //将上传的文件块保存
            public void SaveChunk(HttpPostedFile targetFile, string FilePath, string ContentRange)
                // var ContentRange = HttpContext.Current.Request.Headers["Content-Range"];
            {
                ...
                var nReadSize = 0;
                nReadSize = targetFile.InputStream.Read(nbytes, 0, 512);
                while (nReadSize > 0)
                {
