...
namespace Qx.Tools.Web.Mvc.Html
{
    public static class DateExtensions
    {
        ...
        {
            var value = htmlHelper.DisplayFor(expression).ToString().ToDateTime().FormatDate();
            ...
            dest.Append("</label>");
            dest.Append("<div class='input-group date form_date'>");
...
int numberOfOneRow = 3, string tipString = "")
        {
            ...
            return DateFor(htmlHelper, expression, tipString, numberOfOneRow);
        }

        #region 兼容old
        [Obsolete("使用DateFor代替")]
